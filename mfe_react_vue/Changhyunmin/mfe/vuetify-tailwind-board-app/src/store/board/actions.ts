...
import { 
 ...
 export type BoardActions = {
    ...
    reqeustBoardDeleteToSpring(context:ActionContext<BoardState, any>, boardId: number): Promise<void>
 ...
 const actions: BoardActions = {
    ...
    async requestBoardListToSpring(context: ActionContext<BoardState, any>): Promise<void>{
        try{
            ...
            alert('requestBoardListToSpring() 문제 발생!')
            throw error
        }
    },
    ...
    async reqeustBoardDeleteToSpring(context: ActionContext<BoardState, any>, boardId: number): Promise<void> {
        try {
            await axiosInst.springAxiosInst.delete(`/jpa-board/${boardId}`)
            alert('삭제 성공!')
        }catch(error) {
            alert('requestBoardDeleteToSpring() 문제발생')
            throw error            
        }
    },  
